...
\subsubsection{Technical Implementation}
This module required complex geospatial rendering. We utilized D3's `geoMercator` projection to translate raw GeoJSON polygon coordinates of India's borders into SVG paths. Overlaid on this map is a Force-Directed Node-Link graph. 
Nodes (representing institutions) are pinned to their actual geographic longitude and latitude. Edges (representing co-publications between institutes) are drawn between them. We heavily customized the D3 `d3.forceSimulation` to prevent nodes from overlapping excessively while still maintaining their general geographic accuracy.

\subsubsection{Visual Encodings}
\begin{itemize}
    \item \textbf{Geography (X, Y)}: Nodes are positioned on the map corresponding to their physical campus locations.
    \item \textbf{Size (Radius)}: Scaled to represent the total research output or total domestic funding received by the institution.
    \item \textbf{Color}: A strict binary color coding: vivid gold for "Premier" institutes (IITs, IISc, AIIMS) and a muted, translucent blue for "State \& Affiliated" colleges.
    \item \textbf{Edges (Thickness)}: The stroke width of the connecting lines represents the volume of joint publications between two institutions.
\end{itemize}

\subsubsection{Interactive Capabilities}
Users can interact with a timeline slider to watch the network grow over time. A comprehensive filter panel allows users to toggle visibility based on institution tier (e.g., hiding all state colleges to view only the premier network). Clicking on any specific node triggers a detailed, glassmorphism-styled sidebar card displaying the institution's precise NIRF ranking, funding allocations, and Q1 publication percentage.

\subsubsection{Extracted Insights}
The resulting map is visually striking and deeply concerning from a policy perspective. The visualization immediately reveals highly centralized "hub corridors" localized entirely around the National Capital Region (NCR), Bengaluru, Mumbai, and Chennai. 

The color and size encodings highlight profound asymmetry. The premier nodes (gold) absolutely dominate the canvas in both size and edge connectivity, acting as massive gravitational centers. They capture a vastly disproportionate share of national R\&D funding and produce the overwhelming majority of India's Q1 publications. In contrast, the state-affiliated colleges exist as a fragmented, isolated periphery. This module visually proves that India's domestic knowledge economy is heavily siloed, highlighting an urgent need for policies designed to bridge the collaborative gap between elite central campuses and the broader state infrastructures.
